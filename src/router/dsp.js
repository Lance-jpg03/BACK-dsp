...
import { saveLog } from "../index.js"; // Added import from main index
import { sendBillingEmail } from "../utils/mailer.js";
...
router.post("/", async (req, res) => {
  try {
    const {
      ...
      Category,
      Establishment,
      userName, // Capture userName from request body
    } = req.body;
    ...
    saveLog({
      ...
      action: "CREATED",
      details: `Created record ${finalCollectionId} for ${DSP || 'Unknown DSP'}`
    });

    // TRIGGER EMAIL LOGIC
    if (Status === "FOR BILLING") {
      sendBillingEmail(req.body); 
    }
...
//   try {
//     const { id } = req.params;
//     const { userName, DSP, Status, ...rest } = req.body; // Separate userName and DSP for logging
...
//       user: userName || "System",
//       action: "UPDATED",
//       details: `Updated record ${id} (${DSP || 'DSP Record'})`
//     });

//     // TRIGGER EMAIL LOGIC
//     if (Status === "FOR BILLING") {
//       sendBillingEmail({ ...req.body, Collection_ID: id });
//     }
...
router.put("/:id", async (req, res) => {
  try {
    ...
    // Ensure the Status itself is included in the database update
    if (Status) {
    ...
    // TRIGGER EMAIL LOGIC
    // Using .toUpperCase() to ensure it triggers even if casing is different
    if (Status && Status.toString().toUpperCase() === "FOR BILLING") {
      await sendBillingEmail({ ...req.body, Collection_ID: id });
    }
...
router.delete("/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const { userName } = req.query; // Typically passed via query param in DELETE
